    ...
    when(
      is_function(transform) ~
        ...
        when(
          ...
          # NAN produced
          filter(., !!.abundance %>% is.nan()) %>%
            nrow() %>%
            gt(0) ~ stop("tidyHeatmap says: you applied a transformation that introduced NaN."),

          # -Inf produced
          pull(., !!.abundance) %>%
            min() %>%
            equals(-Inf) ~ stop("tidyHeatmap says: you applied a transformation that introduced negative infinite .value, was it log? If so please use log1p."),
          ~ (.)
        ),
      ~ (.)
    ) %>%
...
add_grouping <- function(my_input_heatmap) {
  ...
  # Number of groups
  how_many_groups <- my_input_heatmap@data %>%
    attr("groups") %>%
    nrow()

  # Number of grouping
  how_many_grouping <- my_input_heatmap@data %>%
    attr("groups") %>%
    select(-.rows) %>%
    ncol()
...
add_annotation <- function(my_input_heatmap,
  ...
  # Number of grouping
  how_many_discrete <- .data_annot %>%
    filter(annot_type == "discrete") %>%
    nrow()
  how_many_continuous <- .data_annot %>%
    filter(annot_type == "continuous") %>%
    nrow()
